...
| 入口 | どのLevelから入るのか |
| 出口 | どのLevelへ出られるのか |
| 取得オブジェクト | Levelで入手できるオブジェクト(案も出してもよい) |
...
docs/Levels/Level0/TheLobby.md

### ルール

- 基本構造は `docs/Levels/Level0/TheLobby.md` と同じにする
- 見出しの順番、項目名、記述粒度をできるだけ揃える
- 新しいレベルを追加する場合も、同じ構成で記述する
- 不要な項目がある場合でも、構造を崩さずに空欄または `未定` として残す
- 独自の項目を追加する場合は、既存構造の末尾に追加する
